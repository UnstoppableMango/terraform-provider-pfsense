...
Edit `cmd/gen-plugingen-config/config.go` → `ConfigFor()` to add an entry to the `Resources` map with the appropriate OpenAPI path/method for each CRUD operation. The downstream Nix pipeline will pick up the change on the next `nix build`.
